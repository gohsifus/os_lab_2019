...
void *ThreadSum(void *args) {
  ...
  struct SumArgs *sum_args = (struct SumArgs *)args + Value;
  sum_args->array = Massive[Value];
  printf("\nArray in ThreadSum");
...
int main(int argc, char **argv) {
 ...
 if(threads_num >= array_size){
  ...
  Massive = malloc(threads_num * sizeof(int));
  for(int i = 0; i< threads_num; i++){
    ...
    printf("\n");
    if(i == 0){
        Massive[i] = malloc(part * sizeof(int));
        for(int j = 0; j < part; j++){
            Massive[i][j] = args[i].array[j];
        }
    }else{
        Massive[i] = args[i].array;
    }
  }
printf("\n");
  ...
  for (uint32_t i = 0; i < threads_num; i++) {
    ...
    total_time = total_time + (start_time.tv_usec - finish_time.tv_usec); 
    total_sum += sum;
  }
